...
describe("bySet", () => {
  beforeAll(() =>
    schemaCfgSpy.mockImplementation((_, key) => ({ [key]: true }))
  );
  ...
  it("passes set to getSchemaFromCfg", () => {
    shared.bySet("setA")(["a"], ["a", "b"], "opt", false);
    expect(schemaCfgSpy).toHaveBeenNthCalledWith(
      1,
      "setA",
      expect.anything(),
      expect.anything(),
  ...
  it("builds object of results", () => {
    expect(shared.bySet("setA")(["a"], ["a", "b"], "opt", false)).toEqual([
      { a: true, b: true },
    ]);
  });
  it("check appends validation checker", () => {
    expect(shared.bySet("setA")(["a"], ["a", "b"], "opt", true)).toEqual([
      expect.anything(),
      checkValidation,
    ]);
  });
