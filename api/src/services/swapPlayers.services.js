...
const {
  ...
  moveArrays,
  getOtherIdx,
} = require("../utils/swapPlayers.utils");

// Get unique IDs (For DB access)
const getUniqueIds = (data, key) =>
  data.reduce(
    (res, entry) => (res.includes(entry[key]) ? res : res.concat(entry[key])),
    []
  );
...
function swapPlayersService(matchData, swapData) {
  ...
  swapData.forEach((data, i) => {
    ...
    if (data.playerIdx === -1)
      throw new Error("Player is not registered for match: " + data.playerid);
