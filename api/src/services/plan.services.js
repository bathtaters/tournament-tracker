...
async function generatePlan(events, voters, settings = {}) {
  ...
  const possibleEvents = Array(slotCount)
    .fill()
    .map(() => []);
  ...
  const eventPrefs = events.reduce(
    (prefs, { id, playercount }) => ({
      ...prefs,
      [id]: {
        ...getEventScores(id, voters, maxEventScore, true),
        _size: playercount,
      },
    }),
    {}
  ...
  for (const eventId of orderedEvents) {
    ...
    const orderedVoters = voters
      .map(({ id }) => id)
      .sort((a, b) => eventPrefs[eventId][b] - eventPrefs[eventId][a]);
  ...
  do {
    ...
    Object.entries(voterData).forEach(([id, { targetEvents }]) => {
      adjustVoters[id] =
        (targetEvents - voterData[id].slots.length) / eventCount;
    });

    for (const id in adjustVoters) {
      const isPos =
        adjustVoters[id] > halfEvent
          ? true
          : adjustVoters[id] < -halfEvent
            ? false
            : null;
      ...
      for (const swapId in adjustVoters) {
        ...
        if (
          swapId === id ||
          adjustVoters[rmvVoter] >= adjustVoters[addVoter] - halfEvent
        )
          continue;
      ...
      schedule[swapSlot].players[schedule[swapSlot].players.indexOf(rmvVoter)] =
        addVoter;
      // Move voter slot
    ...
    adjustTotal > oneEvent &&
    (lastTotal == null || adjustTotal < lastTotal)
  );

  await updateProg(95, 100);

  // Build arrays of "event" update data
  const addEvents = schedule
    .filter(Boolean)
    .map((slot, idx) => slotToEvent(slot, idx, slotsPerDay, dates[0]));
  const rmvEvents = allEvents
    .filter((evId) => addEvents.every(({ id }) => id !== evId))
    .map(resetEvent);
