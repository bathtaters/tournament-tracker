...
function getSchema(key, typeStr, limits, isIn, forceOptional = false) {
  ...
  if (type[2] && type[1] !== "string")
    logger.warn("* is ignored w/ non-string type: ", type[0]);
  ...
  if (type[4]) {
    ptr.optional = {
      options: { values: type[1] === "string" && !zeroMin ? "falsy" : "null" },
    };
  } else {
    ptr.exists = { errorMessage: errorText.exists };
    ptr.optional = {
      options: {
        values: type[1] === "string" && !zeroMin ? "falsy" : "undefined",
      },
    };
  }
  ...
  if (type[3]) {
    ...
    ptr.isArray = {
      options: arrLimit,
      errorMessage: errorText.limit(arrLimit || "array"),
    };
    ...
    if (type[3] === "[?]") ptr.optional = { options: { values: "null" } };
  ...
  if (limits)
    limits = {
      options: limits,
      errorMessage: errorText.limit(limits, type[1] === "string"),
    };
  ...
  switch (type[1]) {
    ...
    case "datetime":
      ptr.isISO8601 = {
        options: dateOptions.time,
        errorMessage: errorText.datetime,
      };
      ptr.toDate = true;
      break;
    case "date":
