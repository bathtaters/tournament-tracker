...
describe("getWLD", () => {
  ...
  it("throws on win > maxwins", () => {
    expect.assertions(5);
    expect(() => getWLD({ wins: [5], maxwins: 0 })).toThrow(
      "Invalid maxwins: 0 => [5]"
    );
    expect(() => getWLD({ wins: [5], maxwins: 3 })).toThrow(
      "Invalid maxwins: 3 => [5]"
    );
    expect(() => getWLD({ wins: [0, 5], maxwins: 0 })).toThrow(
      "Invalid maxwins: 0 => [0,5]"
    );
    expect(() => getWLD({ wins: [0, 5], maxwins: 3 })).toThrow(
      "Invalid maxwins: 3 => [0,5]"
    );
    expect(() => getWLD({ wins: [3, 5], maxwins: 3 })).toThrow(
      "Invalid maxwins: 3 => [3,5]"
    );
  });
});
...
describe("calcRates", () => {
  ...
  it("strips game draws for rateCalc", () => {
    ...
    expect(rateSpy).toHaveBeenCalledTimes(2);
    expect(rateSpy).toHaveBeenNthCalledWith(
      2,
      undefined,
      ["GW", "GL"],
      undefined
...
describe("calcOpps", () => {
  ...
  it("works with no opps", () => {
    expect(calcOpps({}, sampleData)).toEqual({
      oppMatch: [NaN],
      oppGame: [NaN],
    });
  });
  it("works with no data", () => {
    expect(calcOpps({}, {}, opps())).toEqual({
      oppMatch: [NaN],
      oppGame: [NaN],
    });
  });
  ...
  it("averages oppMatches", () => {
    expect(calcOpps({}, sampleData, opps("a")).oppMatch[0]).toBeCloseTo(
      0.5 / 2
    );
    expect(calcOpps({}, sampleData, opps("b")).oppMatch[0]).toBeCloseTo(
      1.0 / 2
    );
    expect(calcOpps({}, sampleData, opps("c")).oppMatch[0]).toBeCloseTo(
      1.5 / 2
    );
    expect(calcOpps({}, sampleData, opps("d")).oppMatch[0]).toBeCloseTo(
      1.5 / 3
    );
  });
  it("averages oppGames", () => {
    expect(calcOpps({}, sampleData, opps("a")).oppGame[0]).toBeCloseTo(
      0.25 / 2
    );
    expect(calcOpps({}, sampleData, opps("b")).oppGame[0]).toBeCloseTo(
      0.75 / 2
    );
...
describe("combineStats", () => {
  ...
  it("matchRecord", () =>
    expect(combined).toHaveProperty("matchRecord", [6, 5, 4]));
  it("gameRecord", () =>
    expect(combined).toHaveProperty("gameRecord", [9, 7, 8]));
...
describe("combineFinal", () => {
  ...
  it("eventids", () =>
    expect(combined).toHaveProperty("eventids", ["d1", "d2", "d3"]));
  it("oppMatch", () => expect(combined).toHaveProperty("oppMatch", [2, 4, 6]));
  it("oppGame", () =>
    expect(combined).toHaveProperty("oppGame", [11, 12, NaN]));
});
...
describe("finalize", () => {
  ...
  beforeEach(() => {
    comboResult = {
      eventids: ["d1"],
      passthrough: true,
      oppMatch: [0.0, 0.5, NaN],
      oppGame: [0.75, 0.25, NaN],
    };
...
describe("rankSort", () => {
  ...
  it("works as sorter", () => {
    ...
    expect(unsortedKeys.sort(rankSort(sampleData))).toEqual([
      "b3",
      "b2",
      "b1",
      "a1",
    ...
    expect(unsortedKeys.sort(rankSort(sampleData, ["b2", "b3"]))).toEqual([
      "b2",
      "b3",
      "b1",
      "a1",
    ...
    expect(unsortedKeys.sort(rankSort(sampleData, ["b2", "b3"], true))).toEqual(
      ["b1", "b2", "b3", "a1", "a2", "a3"]
    );
  });
