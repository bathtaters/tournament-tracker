...
describe('getCombinationN', () => {
  it('includes all possible combinations', () => {
    ...
    let combos = Array(comboCount).fill(null).map((_, c) => 
      getCombinationN(c, array, 3, comboCount)
    );
...
describe('getUniqueCombinations', () => {
  it('includes all unique combinations', () => {
    ...
    expect(combos).toContainEqual([[1, 2], [3, 4]]);
    expect(combos).toContainEqual([[1, 2], [5, 6]]);
    expect(combos).toContainEqual([[3, 4], [5, 6]]);
    expect(combos).toContainEqual([[1, 3], [5, 6]]);
    
    // Overlapping elements are not included
    expect(combos).not.toContainEqual([[1, 2], [1, 3]]);
    expect(combos).not.toContainEqual([[3, 4], [1, 3]]);
  });
