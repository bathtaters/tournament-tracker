  ...
  (obj) => {
    ...
    for (const entry of obj) {
      ...
      if (!(resKey in result)) {
        ...
        // Duplicate keys if combo is TRUE
        result[resKey].push(value);
      } else {
        // Duplicate keys if combo is FALSE
...
exports.customMax = (items, getValue) => {
  if (typeof items !== "object" || !items)
    throw new Error(`customMax expected iterable: ${items}`);
  ...
  for (const key in items) {
    const val = getValue
      ? getValue(items[key], isArr ? +key : key)
      : items[key];
    if (val == null) continue;
    else if (typeof val !== "number")
      throw new Error(`getValue expected to return number: ${val}`);
 ...
 */
exports.getDayCount = (startDate, endDate) =>
  1 + Math.abs(endDate - startDate) / oneDay;
