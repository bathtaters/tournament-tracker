...
describe('newSessionID', () => {
    const uuidRegex = /^[0-9a-fA-F]{8}\b-(?:[0-9a-fA-F]{4}\b-){3}[0-9a-fA-F]{12}$/;
...
describe('stripPassword', () => {
    it('strips out password field', () => {
        expect(utils.stripPassword({ test: '1', password: 2 }))
            .not.toHaveProperty('password');
    });
    it('strips out session field', () => {
        expect(utils.stripPassword({ test: '1', session: false }))
            .not.toHaveProperty('session');
    });
    it('passes other fields', () => {
        expect(utils.stripPassword({ a: 0, b: '2', c: true }))
            .toEqual({ a: 0, b: '2', c: true });
    });
});
...
describe('passwordsMatch', () => {
    ...
    it('returns true on match', () => {
        expect(utils.passwordsMatch('gwR83S-o2N_qpAl','gwR83S-o2N_qpAl')).toBe(true);
    });
    it('returns false on mismatch', () => {
        expect(utils.passwordsMatch('gwR83S-o1N_qpAl','gwR83S-o2N_qpAl')).toBe(false);
    });
});
...
describe('encryptPassword', () => {
    ...
    it('allows non-base64url characters on input', async () => {
        const pw = await utils.encryptPassword('~*-!"\'\\&%*@_!#)@($%&#$&([]{};";?/.<.|\\`');
        expect(pw).toMatch(b64urlRegex);
    });
});