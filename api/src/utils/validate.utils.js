  ...
  sanitize: {
    options: (value) => {
      ...
      intervalKeys.forEach((key) => {
        if (value[key] || value[key] === 0) result[key] = Number(value[key]);
      });
      result.toPostgres = function () {
        return intervalString(this);
      };
      return result;
    },
  },
};
const isDigitOrNull = (value) =>
  typeof value === "number"
    ? true
    : value == null
      ? true
...
const ESC_START = "&",
  ESC_END = ";",
  ESC_MAX_INT_LEN = 5; // Escaping params
exports.escapedLength = ({ options, errorMessage }) => ({
  ...
  options: (value) => {
    ...
    if (reserve) count += reserve;
    return (
      (!options.min || count >= options.min) &&
      (!options.max || count <= options.max)
    );
  },
});
