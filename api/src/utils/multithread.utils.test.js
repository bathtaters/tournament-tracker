...
jest.mock("worker_threads", () => {
  class Worker {
    ...
    constructor(...args) {
      ...
      this.emit = jest
        .fn()
        .mockImplementation((event, arg) => this._events[event](arg));
...
describe("spawnAsync", () => {
  it("throws error if process is already active", () => {
    ...
    expect(() => spawnAsync("test-id", "test-path", { data: "test" })).toThrow(
      "Process already active: test-id"
    );
  });
  ...
  it("sets up event handlers", () => {
    ...
    expect(Worker._instances[0].on).toHaveBeenCalledWith(
      "message",
      expect.any(Function)
    );
    expect(Worker._instances[0].on).toHaveBeenCalledWith(
  ...
  it("terminates on exit with code", () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      "Thread test-id ended with exit code: 1"
    );
...
describe("abort", () => {
  ...
  it("throws error if process not found", async () => {
    expect.assertions(1);
    await expect(abort("non-existent-id")).rejects.toThrow(
      "Process not found: non-existent-id"
    );
  });
});
