...
describe("arrToObj", () => {
  ...
  it("converts single object to objArray", () => {
    expect(arrToObj("key", { delKey: 0 })(testArray[0])).toEqual({
      a: expectedResult.a,
    });
  });
  ...
  it("alerts on missing key", () => {
    ...
    expect(warnSpy).toHaveBeenCalledTimes(1);
    expect(warnSpy).toHaveBeenCalledWith("Entry is missing key:", "key", {
      value: 4,
    });
  });
  ...
  it("uses valKey as value", () => {
    expect(arrToObj("key", { valKey: "value" })(testArray)).toEqual({
      a: 1,
      b: 2,
      c: 3,
    });
...
describe("insertSorted", () => {
  ...
  it("works in reverse", () => {
    expect(insertSorted(testArray.reverse(), 3, (a, b) => a > b)).toEqual([
      8, 6, 4, 3, 2,
    ]);
  });
...
describe("midOut", () => {
  it("odd array", () => {
    expect([...midOut(["a", "b", "c", "d", "e"])]).toEqual([2, 3, 1, 4, 0]);
  });
  it("even array", () => {
    expect([...midOut(["a", "b", "c", "d"])]).toEqual([1, 2, 0, 3]);
  });
  it("length number", () => {
    expect([...midOut(7)]).toEqual([3, 4, 2, 5, 1, 6, 0]);
  });
  it("return asValue instead of key", () => {
    expect([...midOut(["a", "b", "c"], true)]).toEqual(["b", "c", "a"]);
  });
  it("empty array", () => {
...
describe("threadSanitize", () => {
  ...
  it("removes other types", () => {
    ...
    expect(threadSanitize({ a: 1, b: func, c: sym })).toEqual({ a: 1 });
    expect(threadSanitize([1, func, sym])).toEqual([1, undefined, undefined]);
  });
});
...
describe("toDateStr", () => {
  ...
  it("passes arrays/objects", () => {
    expect(toDateStr([1, "2", true])).toEqual([1, "2", true]);
    expect(toDateStr({ a: 1, test: "2", x: true })).toEqual({
      a: 1,
      test: "2",
      x: true,
    });
    expect(toDateStr({ a: 1, test: [1, "2", true], x: true })).toEqual({
      a: 1,
      test: [1, "2", true],
      x: true,
    });
    expect(toDateStr([1, { a: 1, test: "2", x: true }, true])).toEqual([
      1,
      { a: 1, test: "2", x: true },
      true,
    ]);
  ...
  it("converts nested dates", () => {
    expect(toDateStr([1, "2", new Date(2021, 3 - 1, 19)])).toEqual([
      1,
      "2",
      "2021-03-19",
    ]);
