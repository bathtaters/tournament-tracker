...
const combinationCount = (arrayLen, slotCount) => slotCount && Math.round(fact(arrayLen) / (fact(slotCount) * fact(arrayLen - slotCount)))
...
const isUnique = (indexes, matches) => {
  ...
  */
  const generateMatches = (array) => array.map(
      (base) => array.map((_,i) => i).filter(
          (i) => array[i].every((item) => !base.includes(item))
      )
  )
...
function* getUniqueCombinations(array, size) {
    ...
    const slotPtr = Array(size).fill(0).map((_,i) => size - i - 1)
  ...
  function* getCombinations(array, slotCount) {
    ...
    // Initialize pointers for each slot (Most active ptr first)
    const slotPtr = Array(slotCount).fill(0).map((_,i) => slotCount - i - 1)
    // Set limits for each pointer
    const limit = slotPtr.map((_,i) => array.length - i - 1)
