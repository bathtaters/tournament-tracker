...
describe('getConnStr', () => {
  ...
  beforeAll(() => { mockRepl.mockImplementation(o=>o); });
  afterAll(() => { mockRepl.mockRestore(); });

  it('gets connStr', () => {
    expect(connectUtils.getConnStr('test', mockServerCfg))
      .toBe(mockServerCfg.connectionString);
  });

  it('gets connStr w/o user', () => {
    const result = connectUtils.getConnStr(null, {connectionString: 'TestConnString'});
  ...
  describe('error handling', () => {
    ...
    it('fails on missing config', () => {
      expect(() => connectUtils.getConnStr(null, {})).toThrow('Server config is invalid');
    });

    it('fails on user not found', () => {
      expect(() => connectUtils.getConnStr('MissingUser')).toThrow('No credentials for MissingUser');
    });
  });
  ...
  describe('external calls', () => {
    ...
    it('calls replFromObj() using serverData', () => {
      ...
      expect(connectUtils.replaceFromObj).toHaveBeenNthCalledWith(1,
    ...
    it('calls replFromObj() using userData', () => {
      ...
      expect(connectUtils.replaceFromObj).toHaveBeenCalledTimes(3);
      expect(connectUtils.replaceFromObj).toHaveBeenNthCalledWith(2,
        mockServerCfg.connectionString,
    ...
    it('calls replFromObj() using envData', () => {
      ...
      expect(connectUtils.replaceFromObj).toHaveBeenCalledTimes(3);
      expect(connectUtils.replaceFromObj).toHaveBeenNthCalledWith(3,
        mockServerCfg.connectionString,
...
describe('retryBlock', () => {
  ...
  it('returns result on success', () => {
    ...
    return expect(connectUtils.retryBlock(mockFunc, [], 1)).resolves
      .toBe('Success');
  });
  ...
  it('throws error if err.code not in retryCodes', () => {
    ...
    return expect(connectUtils.retryBlock(mockFunc, [], 1, 0, [10])).rejects
      .toEqual({ message: 'Fail', code: 20, stack: expect.any(String) });
  });

  it('doesn\'t throw if err.code is in retryCodes', () => {
    mockFunc.mockRejectedValueOnce({ message: 'Fail', code: 10 });

    return expect(connectUtils.retryBlock(mockFunc, [], 2, 0, [10])).resolves.toEqual([]);
  });
...
describe('replaceFromObj', () => {
  ...
  it('missing obj returns input str', () => {
    expect(connectUtils.replaceFromObj(
      'test'
    )).toBe('test');
  });
  ...
  it('replaces case-insensitive', () => {
    expect(connectUtils.replaceFromObj(
      'test ES', {'es':'ar'}, { caseI: true }
    )).toBe('tart ar');
  });
  
  it('escapes replacements', () => {
    expect(connectUtils.replaceFromObj(
      'test es', {'test':'"test;'}, { esc: true }
    )).toBe('%22test%3B es');
  });
