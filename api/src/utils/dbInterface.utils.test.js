...
describe("intervalString", () => {
  it("converts object to string", () => {
    expect(
      ...
      "2 years 3 months 4 days 5 hours 16 minutes 27 seconds 389 milliseconds"
    );
    expect(intervalString({ seconds: 123 })).toBe("123 seconds");
    expect(intervalString({ seconds: 123, milliseconds: 456 })).toBe(
      "123 seconds 456 milliseconds"
    );
  });
  ...
  it("ignores non-interval keys", () => {
    expect(intervalString({ days: 4, minutes: 6, foo: 11 })).toBe(
      "4 days 6 minutes"
    );
  });
  it("ignores non-numeric values", () => {
    expect(intervalString({ days: 14, minutes: "6", seconds: 55 })).toBe(
      "14 days 55 seconds"
    );
  });
...
describe("strTest", () => {
  it("throws injected strings", () => {
    expect.assertions(2);
    expect(() => strTest("injection attempt")).toThrow(
      "Possible SQL injection: injection attempt"
    );
    expect(() => strTest("injection;")).toThrow(
      "Possible SQL injection: injection;"
    );
  });
  ...
  it("accepts arrays, throws on injected", () => {
    expect.assertions(1);
    expect(() => strTest(["safe", "injection;"])).toThrow(
      "Possible SQL injection: injection;"
    );
  });

  it("accepts arrays, passes non-injected", () => {
    expect(strTest(["safe", 22, "pass"])).toEqual(["safe", "22", "pass"]);
  });
});
...
describe("queryValues", () => {
  ...
  it("flattens result", () => {
    expect(queryValues(testArray, ["valA", "valB"])).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
  });
  ...
  it("returns undefined on missing keys", () => {
    expect(queryValues(testArray, ["valA", "missing"])).toEqual([
      1,
      undefined,
      3,
      undefined,
...
describe("sqlSub", () => {
  it("substitutes indexes", () => {
    expect(sqlSub("TEST SQL result = $1 AND true; ", ['"one"'])).toBe(
      'TEST SQL result = "one" AND true; '
    );
    expect(
      sqlSub("TEST SQL result = $1 AND next = $3 OR final = $2 AND false; ", [
        '"one"',
        '"two"',
        '"three"',
      ])
    ).toBe(
      'TEST SQL result = "one" AND next = "three" OR final = "two" AND false; '
    );
  });
  it("ignores indexes > args.length and non-numeric", () => {
    expect(sqlSub("TEST result = $1 AND next = $3;", ['"one"'])).toBe(
      'TEST result = "one" AND next = $3;'
    );
    expect(
      sqlSub("TEST SQL result = $1 AND next = $N OR final = $2;", [
        '"one"',
        '"two"',
        '"three"',
      ])
  ...
  it("allows skipping indexes", () => {
    expect(
      sqlSub("TEST SQL result = $1 AND next = $3;", [
        '"one"',
        '"two"',
        '"three"',
      ])
...
describe("getReturn", () => {
  ...
  it("passes result if no/falsy rows", () => {
    ...
    expect(getReturn({ other: "result", rows: null })).toEqual({
      other: "result",
      rows: null,
    });
    expect(getReturn([{ other: "result", rows: null }])).toEqual([
      { other: "result", rows: null },
    ]);
  });
