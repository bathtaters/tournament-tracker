...
const createPlayer = (req, res) => players.add(matchedData(req), req.session.user || req.sessionID).then(res.sendAndLog);
const removePlayer = (req, res) => players.rmv(matchedData(req).id, req.session.user || req.sessionID).then(res.sendAndLog);
...
const updatePlayer = async (req, res) => {
  ...
  return players.set(id, body, req.session.user || req.sessionID).then(res.sendAndLog);
}

// Reset Session
const resetPassword = async (req, res) => {
  const { id } = matchedData(req);
  const session = await players.resetLogin(id, req.session.user || req.sessionID);
