...
const resetAllCredits = (includeFinished = false) => includeFinished ?
  async function resetAllCredits(req) {
    ...
    for (const id of players) {
      credits[id] = 0
      await player.set(id, { credits: 0 }, req.session.user || req.sessionID)
    }
  ...
  async function resetAllCredits(req, res) {
    ...
    for (const id in credits) {
      await player.set(id, { credits: credits[id] }, req.session.user || req.sessionID)
    }
    return res.sendAndLog(credits)
  }
...
const setCredits = (undo = false) =>
  async function setCredits(req, res) {
    ...
    for (const { id, credits } of allPlayers) {
      ...
      await player.set(id, { credits: credits + (undo ? -eventCredits[id] : eventCredits[id]) }, req.session.user || req.sessionID)
    }
