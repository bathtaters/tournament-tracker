...
async function logout(req, res, next) {
  if (!req.session.user)
    return next({ status: 400, message: "User is not logged in." });
...
async function player(req, res, next) {
  ...
  return player
    ? res.sendAndLog(player)
    : next({ status: 204, message: "Player not found" });
}

// Fetch session cookie
async function fetch(req, res, next) {
  if (!req.sessionID)
    return next({ status: 500, message: "Server failed to start a session." });
...
const setupStatus = async (_, res) => {
  ...
  if (typeof adminCount !== "number")
    return res.sendAndLog({ error: "Server error" });
