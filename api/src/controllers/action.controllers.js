...
async function swapPlayers(req, res) {
  ...
  // Write changes
  const result = await match.updateMulti(matchData, req.session.user);
...
async function nextRound(req, res) {
  ...
  // Create matches
  const ret = await event.pushRound(eventid, round, matches, req.session.user);
...
async function prevRound(req, res) {
  ...
  await event.popRound(id, round, req.session.user);
