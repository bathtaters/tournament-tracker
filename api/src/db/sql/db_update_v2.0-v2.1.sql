...
ALTER TABLE "event"
    ADD COLUMN planidx SMALLINT DEFAULT 0;
WITH seed AS (SELECT id, row_number() OVER (ORDER BY id) AS idx
              FROM "event"
              WHERE plan::BOOLEAN)
UPDATE "event"
SET planidx = seed.idx
FROM seed
WHERE "event".id = seed.id;

-- Replace old column
ALTER TABLE "event"
    DROP COLUMN plan;
ALTER TABLE "event"
    RENAME COLUMN planidx TO plan;


-- Add index column to VOTERS --
--------------------------------
ALTER TABLE "voter"
    ADD COLUMN idx SMALLINT DEFAULT 0;
