...
function query(text, args = [], splitArgs = null) {
  ...
  return db.runOperation((client) =>
    Promise.all(text.map((q, i) => client.query(q, splitArgs ? args[i] : args)))
  );
}
...
async function loadFiles(pathArray) {
  ...
  let sqlFileText = await Promise.all(
    pathArray.map(
      (path) =>
        path &&
        fs
...
module.exports = {
  ...
  execFiles: (pathArray) =>
    loadFiles(pathArray).then((text) => query(text, [])),
};
