...
const teamQueries = {
  addMember:
    "UPDATE player SET members = members || $1 WHERE isteam = TRUE AND id = $2;",
  rmvMember:
    "UPDATE player SET members = array_remove(members, $1) WHERE isteam = TRUE AND id = $2;",
  replMember:
    "UPDATE player SET members = array_replace(members, $1, $2) WHERE isteam = TRUE AND id = $3;",
  replMember2:
    " UPDATE player SET members = array_replace(members, $2, $1) WHERE isteam = TRUE AND id = $4;",
...
const add = (members, withName = null, req) =>
  db
    .operation(async (client) => {
      const memberArr = await db
        .getRows(
          "player",
          teamQueries.checkPlayers,
          [members],
          ["id", "isteam"],
          0,
          0,
      ...
      if (
        memberArr?.length !== members.length ||
      ...
      return log.addRows(
        "player",
        { members, isteam: true, name: withName },
        req,
        { client }
      );
    })
...
const rmv = (teamid, req) =>
  log.rmvRows("player", [teamid], teamQueries.selectTeam, req);

// Switch in/out members
const addMember = (teamid, playerid, req) =>
  // Add checks that members aren't teams
  log.query(
    teamQueries.addMember,
    [playerid, teamid],
    (data, error) => ({
...
const replaceMember = (teamid, oldPlayerId, newPlayerId, req) =>
  ...
  log.query(
    teamQueries.replMember,
    [oldPlayerId, newPlayerId, teamid],
    (data, error) => ({
      dbtable: log.TableName.PLAYER,
      action: log.LogAction.UPDATE,
      tableid: data?.id || teamid,
...
const swapMembers = (playeridL, teamidL, playeridR, teamidR, req) =>
  log.query(
