...
const get = (matchId, detail = false, fields = null) =>
  db.getRow("match" + (detail ? "Detail" : ""), matchId, fields);

const getMulti = (matchIds, detail = false, fields = null) =>
  db.operation((client) =>
    Promise.all(
      matchIds.map((id) =>
...
const getByEvent = (eventid) =>
  db.getRow("matchDetail", eventid, null, { idCol: "eventid", getOne: false });

// Get all match details for stats
const getAll = (completed = true) =>
  db.getRows(
    "matchDetail",
...
const dropPlayer = (id, player, drop, req) =>
  log
    .query(
...
const updateMulti = (dataArray, req) =>
  db.operation((client) =>
    Promise.all(
      dataArray.map((data) =>
        data.id
          ? log.updateRows("match", data.id, filtering(data, ["id"]), req, {
              client,
            })
          : log
              .addEntries(
