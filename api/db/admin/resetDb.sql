...
-- CREATE VIEW matchForDraftPlayer (
--     id, draftId,
--     players, round,
--     isDone, isBye,
--     winners
-- ) AS SELECT 
--     match.id, match.draftId,
--     match.players, match.round,
--     match.reported, -- isDone
--     array_length(match.players, 1) = 1, -- isBye
--     -- winners
--     IF(
--         match.reported,
--         array_agg(xWins.player)
--             FILTER(WHERE xWins.win = xWins.maxwins),
--         NULL
--     )
-- FROM match@draft_idx
-- JOIN (SELECT id,
--         unnest(players) player,
--         unnest(wins) win,
--         (SELECT max(w) FROM unnest(wins) w) maxwins
--     FROM match@draft_idx ORDER BY id) xWins 
--     USING (id)
-- GROUP BY match.id
...
-- CREATE VIEW matchForMatchPlayer (
--     id, draftId, players, round,
--     isDone, wins,
--     draws, games,
--     winners
-- ) AS SELECT 
--     match.id, match.draftId, match.players, match.round,
--     match.reported, -- isDone
--     json_object(match.players::STRING[], match.wins::STRING[]), -- wins
--     match.draws,
--     SUM(win) + match.draws, -- games
--     -- winners
--     IF(
--         match.reported,
--         array_agg(player) FILTER(WHERE win = maxwins),
--         NULL
--     )
-- FROM match@draft_idx,
--     unnest(players) player,
...
--     player, m.id,
--     m.draftId, m.round,
--     (m.wins->>(player::STRING))::SMALLINT, -- wins
--     m.draws, -- draws
--     m.games, -- count
--     -- winner
--     IF(m.isDone,
--         IF(array_length(m.winners,1) = 1,
--             m.winners[1]::STRING, 'DRAW'
--         ), NULL
--     )
-- FROM matchForMatchPlayer m
-- CROSS JOIN unnest(m.players) player;
...
--     -- matchWins
--     (COUNT(m.id)
--     FILTER(WHERE m.reported
--         AND array_length(m.winners,1) = 1
--         AND playerId = m.winners[1]::STRING))::SMALLINT,
--     -- matchDraws
--     (COUNT(m.id)
--     FILTER(WHERE m.reported
--         AND array_length(m.winners,1) != 1))::SMALLINT,
--     -- matchCount
--     (COUNT(m.id) FILTER(WHERE m.reported))::SMALLINT,
--     -- gameWins
--     SUM((m.wins->>(playerId))::SMALLINT),
--     -- gameDraws
--     SUM(m.draws),
--     -- gameCount
--     SUM(m.games),
...
-- FROM draft
-- JOIN matchFull m
--     ON m.draftId = draft.id,
--     json_object_keys(m.wins) playerId,
--     json_object_keys(m.wins) oppId
--     WHERE oppId != playerId
-- GROUP BY playerId, draft.id;

-- CREATE VIEW pageSchedule
-- CREATE VIEW pageDraft
-- CREATE VIEW pageMatch
-- CREATE VIEW pagePlayer