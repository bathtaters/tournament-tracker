...
export const PicColumnStyle = ({ children }) => (
  <div className="w-36">{children}</div>
);

export function ProfilePicStyle({ children }) {
  return (
    <div className="w-full h-full mask mask-squircle bg-neutral flex">
      {children}
    </div>
  );
}
...
export function PlayerDataStyle({ children }) {
  return (
    <div className="grow shrink w-screen sm:max-w-md my-1">
      <form className="grid grid-flow-row grid-cols-[max-content_1fr_auto] items-center px-4 gap-3">
        {children}
      </form>
    </div>
  );
...
export const ResetLinkStyle = (props) => (
  <a className="link link-secondary" {...props}>
    Reset Password
  </a>
);
