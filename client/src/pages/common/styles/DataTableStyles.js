...
export function HeaderStyle({ label, span = 1, className = "" }) {
  return (
    <th className={className} colSpan={span}>
      {label}
    </th>
  );
}
...
export function CellStyle({
  ...
  return (
    <td
      className={`relative ${baseClass} text-${size} font-${weight} text-${align} text-${color} ${className}`}
      colSpan={span}
    >
      {to && (
        <Link
...
export function MissingStyle({ colCount = 1, children }) {
  return (
    <tr>
      <td
        className="italic opacity-90 text-center font-thin"
        colSpan={colCount}
      >
        {children}
      </td>
