...
export function NotesOverlayStyle({ visible, onClick, children }) {
  ...
  return (
    <div
      className="absolute top-0 bottom-0 left-0 right-0 z-10 rounded-lg flex bg-base-200 opacity-30"
      onClick={onClick}
    >
      <div className="m-auto pb-1 text-base-content italic font-thin cursor-pointer">
        {children}
      </div>
    </div>
  );
