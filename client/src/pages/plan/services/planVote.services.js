...
import { arrInsert, arrRemove, arrShift, arrSwap, dateRangeList, trimFalsy } from "./plan.utils"
...
export function useVoterScores(voters, events, players, { plandates }, skip) {
    ...
    return useMemo(() => {
        ...
        const unvotedPenalty = -2,
            scores = Object.fromEntries(Object.keys(voters).map((id) => [id, {
            ...
            totals = {
                ...
                events: Math.max(...Object.values(voters).map(({ events }) => events.length)),
            },
            errors = []
        ...
        for (const day of schedule) {
            for (const eventId of day.events) {
                ...
                for (const player of events[eventId].players) {
                    ...
                    scores[player].rankSum += rank === -1 ? unvotedPenalty : totals.events - rank
        ...
        for (const player in scores) {
            ...
            scores[player].offDays = voters[player].days.filter((day) => totals.days.includes(day)).length
        ...
        for (const player in scores) {
            ...
            scores[player].weighted = Math.min(1.0, Math.max(0, scores[player].score * normalizedWeight))
        }
