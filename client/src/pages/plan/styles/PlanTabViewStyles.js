...
export const ViewErrors = ({ errors }) =>
  !errors?.length ? null : (
    <div className="flex flex-wrap justify-center items-center gap-2 mt-4">
      ...
      {errors.map((msg) => (
        <div className="badge badge-error font-light text-sm">{msg}</div>
      ))}
    </div>
  );

export const ViewCellSectionStyle = ({
  header,
  children,
  open = true,
  emptyHeader,
  ...
  return (
    <details
      ...
      open={isEmpty ? false : open}
    >
      <summary className="collapse-title p-1 after:top-4!">
        <h5>
          {(isEmpty ? emptyHeader || header : header) || emptyPlaceholder}
        </h5>
      </summary>
  ...
  >
    ...
    {(isRegistered || isUnvoted) && (
      <div
        ...
        title={isUnvoted ? "Registered, didn't vote for" : "Registered"}
      />
    )}
  </li>
);
