...
export function createUpdate(body, { dispatch, getState, queryFulfilled }) {
  ...
  const updatePlayer = dispatch(fetchApi.util.updateQueryData('player', undefined, draft => { draft[id] = body; }));
  const updateStats  = dispatch(fetchApi.util.updateQueryData('stats', undefined, draft => { draft.ranking.push(id); }));
  queryFulfilled.catch(() => { updatePlayer.undo(); updateStats.undo(); }); // rollback
};

export function deleteUpdate(id, { dispatch, queryFulfilled }) {
  const updatePlayer = dispatch(fetchApi.util.updateQueryData('player', undefined, draft => { delete draft[id]; }));
  const updateStats  = dispatch(fetchApi.util.updateQueryData('stats', undefined, draft => {
