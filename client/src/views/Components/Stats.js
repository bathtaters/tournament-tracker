...
function Stats(props) {
  ...
  return pug`
    .relative
      .grid.grid-flow-row.grid-cols-stats.gap-x-2.gap-y-1.items-center.px-2
        ...
        each pid, idx in props.ranking
          Fragment(key=pid+"S")
            ...
            Link.text-lg.font-normal.text-left(to="/profile/"+pid)= props.players[pid].name
      ...
      .grid.grid-flow-row.grid-cols-1.gap-x-2.gap-y-1.items-center.absolute.top-0.left-0.right-0.bottom-0.z-0
        .w-full.h-full.opacity-75.mb-2.bg-none

        each pid, idx in props.ranking
          Link.w-full.h-full.px-2.opacity-0.base-bgd-inv(to="/profile/"+pid className="hover:opacity-25" key=pid+"L")
  `;
}
